...
- \usepackage{fancyhdr}
- \pagestyle{fancy}
- \fancyhead[RE,RO]{\includegraphics[width=7cm]{/srv/shiny-server/0408/CCEB.png}}
...
### Please check https://www.xmeta.org/ to explore more features. 

## 1.2 About the method and dataset
### According to the uploaded dataset, this is a report for _Univariate Meta-analysis_ with _Continuous Outcomes_. 
...
\vspace{0.5cm}
```{r,echo=FALSE}
mc1 <- metacont(Ne, Me, Se, Nc, Mc, Sc,data = params$n,studlab = paste(author, year))
mc1
```
\vspace{1cm}
...
forest(mc1, xlab ="Difference in mean response (intervention - control)
...
The value of ${tau}^2$ is `r (mc1$tau)^2`; the value of $I^2$ is `r round((mc1$I2)*100,digit =2)`$\%$.
...
funnel <- funnel(mc1,level=0.95,contour=c(0.9, 0.95, 0.99), shade=c("darkgray", "gray", "lightgray"))
