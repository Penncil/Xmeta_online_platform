uniMod <- function(input,output,session){
  ...
  output$sub_group <- renderPlot({
    ...
    dataset = data()
    mc_s <- metacont(Ne, Me, Se, Nc, Mc, Sc,data = dataset,studlab = paste(author, year),byvar = dataset[,9])
    forest(mc_s)
  ...
  output$meta_reg <- renderPlot({
    ...
    dataset = data()
    mc1 <- metacont(Ne, Me, Se, Nc, Mc, Sc,data = dataset,studlab = paste(author, year))
    mc1_reg <- metareg(mc1, year)
    bubble(mc1_reg)
  })
  
  output$report <- downloadHandler(
    # For PDF output, change this to "report.pdf"
    filename = "report_NMA_con.pdf",
    content = function(file) {
      # Copy the report file to a temporary directory before processing it, in
      # case we don't have write permissions to the current working dir (which
      # can happen when deployed).
      tempReport <- file.path(tempdir(), "report_uni_con.Rmd")
      file.copy("report_uni_con.Rmd", tempReport, overwrite = TRUE)
...
###########################################################
###########################################################
uniModUI <- function(id){
  ns <- NS(id)
  navlistPanel(
    tabPanel("Upload File (UMA with continuous outcomes)",
             sidebarLayout(
               mainPanel(
    ...
    tabPanel('Subgroup Analysis',plotOutput(ns('sub_group'))),
    tabPanel('Meta-regression',plotOutput(ns('meta_reg'))),
